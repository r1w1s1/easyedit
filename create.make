...
# make decisions about use of curses/new_curse.
#
# Modern ee uses wide-character curses interfaces (wget_wch, KEY_CODE_YES,
# wcwidth). Prefer system ncursesw when available.

CURSESLIB="-lcurses"

if [ -f /usr/include/ncursesw/curses.h -o -f /usr/include/ncurses.h ]
then
	TARGET="curses"
	curses="-DHAS_NCURSES -D_XOPEN_SOURCE=700 -D_DEFAULT_SOURCE"
	CURSESLIB="-lncursesw"
...
echo "LIBS =	$CURSESLIB" >> make.local
echo "" >> make.local
echo "" >> make.local
...
curses :	ee.c
	cc ee.c -o ee \$(DEFINES) \$(CFLAGS) \$(LIBS)
